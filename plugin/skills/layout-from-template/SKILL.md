...
**Template item width is the invariant; height and position flex.** Keep each template item's width (`title`, `legend`, `info_block`, `scalebar`) — the shared info-column width is what holds the layout aligned. Height and vertical position may change to fit content: the legend and its `legend_note` grow or shrink in **height** and reflow, the title box grows downward for a longer title. Keep the legend's **`resizeToContents` OFF** (the template ships it off) — it auto-fits the box to content and silently *narrows the width*, breaking the column; never call `setResizeToContents(True)`. Size the legend/note height explicitly and hold them at the column width. A **changed width** is the drift signal, not a taller or shifted item. Exceptions exist (a deliberately wider block), but they are a conscious choice, never incidental.
