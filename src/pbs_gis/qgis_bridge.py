...
This module is **optional** — pbs_gis stays headless-capable.  If the
...
1. ``pip install pbs-gis[qgis]`` (or ``pip install qgis-mcp`` directly).
...
def add_wms_layer(
    ...
    * Scale-based visibility is configured from the layer's
    ...
    """
    from pbs_gis.wms import get_wms_layer_metadata  # avoid import at module load
    ...
    code = (
        ...
        "_existing = None\n"
        "for _l in QgsProject.instance().mapLayers().values():\n"
        "    if _l.customProperty('pbs_gis/wms_id') == _wms_id:\n"
        "        _existing = _l\n"
        "        break\n"
        ...
        "    else:\n"
        "        _layer.setCustomProperty('pbs_gis/wms_id', _wms_id)\n"
        "        if _enable_scale:\n"
