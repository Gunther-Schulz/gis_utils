...
Usage:
    ...
    python -m pbs_gis.runner

    # Or specify project path:
    python -m pbs_gis.runner /path/to/project

    # Run a single step:
    python -m pbs_gis.runner --step "Extract DXF layers"

    # Dry run (show execution plan):
    python -m pbs_gis.runner --dry-run
"""
...
def _run_recipe_step(step: dict, project_dir: Path) -> bool:
    ...
    # Multi-layer recipe
    if layer_aliases:
        from pbs_gis.recipes import run_multi_layer_recipe
    ...
    # Single-layer recipe
    from pbs_gis.recipes import run_recipe
    output = step.get("output")
    output_path = project_dir / output if output else None
    try:
        run_recipe(
...
def _maybe_write_manifest(step: dict, project_dir: Path) -> None:
    ...
    if not step.get("publiziert"):
        return
    from pbs_gis.manifest import werkzeug_id, write_manifest
...
# Discover available templates via the pbs-gis MCP tool `list_templates`
...
def init_project(project_dir: str | Path) -> None:
    ...
    print(f"\nGIS skills and MCP tools provided by the pbs-gis Claude Code plugin.")
    print(f"Install: claude plugin marketplace add Gunther-Schulz/pbs_gis")
...
def _run_check_recipes(recipe_name: str | None, project_dir: str) -> None:
    ...
    from pbs_gis.recipes import check_recipe_layers, list_recipes, load_recipe
