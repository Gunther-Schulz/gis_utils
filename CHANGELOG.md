...
- **GeoPackage is the default vector output** — codified in the always-on `gis-safety` skill: write `.gpkg` (`to_file(path, driver="GPKG")`) under `Geodaten/`; Shapefile only when an external recipient requires it (no 10-char field truncation, single-file, multi-layer, UTF-8/NULL). The `workflow-authoring` layout table marks `Geodaten/` as GeoPackage. The library already writes GPKG in nearly all paths; this makes the convention explicit so projects stop emitting `.shp`.
